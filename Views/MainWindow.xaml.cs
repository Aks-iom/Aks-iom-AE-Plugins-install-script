...
public partial class MainWindow : Window
{
    ...
    internal void DeleteConfigBtn_Click(object? sender = null, RoutedEventArgs? e = null)
    {
        ...
        if (_currentConfig.IsBuiltIn)
        {
            AppMessageDialog.Info(this, L10n.T("dlg.del_config.title"),
                L10n.T("dlg.del_config.builtin_locked"));
            return;
        }
    ...
    private async Task InstallAsync(List<PluginRow> rows)
    {
        ...
        bool overwrite = summary.Overwrite;
        if (!overwrite)
        {
            var skipped = rows.Where(r => r.IsInstalled).ToList();
            foreach (var s in skipped)
                AppendLog(L10n.T("log.skip_installed", s.Model.Name));
            rows = rows.Where(r => !r.IsInstalled).ToList();
            if (rows.Count == 0)
            {
                ProgressText.Text = L10n.T("progress.idle");
                return;
            }
        }
        ...
        _sessionJsonLogPath = null;  // новый JSONL-лог на каждый сеанс установки

        // Открываем секцию попытки в session-log (инкрементит TotalAttempts).
        var attemptMeta = new AttemptMeta
        {
            AeVersion = ae.Version,
            AeInstallPath = ae.InstallPath ?? "",
            Plugins = rows.Select(r => string.IsNullOrEmpty(r.Model.Version)
                ? r.Model.Name
                : $"{r.Model.Name} v{r.Model.Version}").ToList(),
            Overwrite = overwrite,
            TelemetryEnabled = App.Progress.Data.IsTelemetryEnabled
        };
        var attemptNo = App.SessionLog.BeginAttempt(attemptMeta);
        var attemptSw = Stopwatch.StartNew();

        // Блокируем показ окна авто-апдейтера на всё время установки плагинов.
        App.Updater.BeginInstallation();

        SetUiBusy(true);
        HideLogEmptyHint();
        var rootTempDir = Path.Combine(Path.GetTempPath(), $"aeplugin_dl_{Guid.NewGuid():N}");
        Directory.CreateDirectory(rootTempDir);
        App.SessionLog.Debug($"root_temp_dir={rootTempDir}");

        AppendLog(L10n.T("log.install_begin", ae.ToString()));
        App.SessionLog.Info($"Install begin — attempt #{attemptNo}, ae={ae}");
        ...
        try
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                var headerTitle = $"[{i + 1}/{rows.Count}] {plugin.Name}"
                    + (string.IsNullOrEmpty(plugin.Version) ? "" : $"  v{plugin.Version}")
                    + (string.IsNullOrEmpty(plugin.Size) ? "" : $"  ·  {plugin.Size}");

                AppendLog("");
                AppendLog("┌─ ▸ " + headerTitle);

                App.SessionLog.Info("── Plugin " + headerTitle);

                try
                {
                    var pluginSrcDir = Path.Combine(rootTempDir, SafeFolder(plugin.Name));
                    Directory.CreateDirectory(pluginSrcDir);
                    App.SessionLog.Debug($"plugin_src_dir={pluginSrcDir}");

                    var files = await DownloadPluginFilesAsync(
                        downloader, plugin, pluginSrcDir, $"{i + 1}/{rows.Count}", ct);

                    if (files.Count == 0)
                    {
                        AppendLog(L10n.T("log.empty_link"));
                        AppendLog("└─ ❌ Не удалось скачать файлы");
                        App.SessionLog.Warn($"Plugin '{plugin.Name}': no files downloaded (empty/invalid link)");
                        fail++; continue;
                    }
                    ...
                    var ctx = new InstallContext
                    {
                        ...
                        UiLog = msg => Dispatcher.Invoke(() => AppendLog(msg, mirrorToSession: false)),
                        SessionSink = (lvl, m) => App.SessionLog.Append(MapLevel(lvl), m),
                        Cancel = ct,
                        JsonLogPath = GetSessionJsonLogPath()
                    };

                    var pluginSw = Stopwatch.StartNew();
                    var success = await Task.Run(() => _engine.Install(stepsJson, ctx, plugin.Version), ct);
                    pluginSw.Stop();
                    var dur = $"{pluginSw.Elapsed.TotalSeconds:0.00}s";
                    if (success)
                    {
                        if (!string.IsNullOrEmpty(plugin.Warning))
                            AppendLog($"│  ⚠ {plugin.Warning}");
                        AppendLog($"└─ ✅ Установлен  ·  {dur}");
                        ok++;
                    }
                    else
                    {
                        AppendLog($"└─ ❌ Установка не завершена  ·  {dur}");
                        fail++;
                    }
                }
                ...
                catch (Exception ex)
                {
                    ...
                    AppendLog("└─ ❌ Установка прервана исключением");
                    App.SessionLog.Exception($"Plugin '{plugin.Name}' failed with unhandled exception", ex);
                    fail++;
                }
            }
        }
        finally
        {
            ...
            _installCts?.Dispose();
            _installCts = null;
            App.Updater.EndInstallation();
        }

        AppendLog("");
        if (cancelled)
        {
            AppendLog(L10n.T("log.install_cancelled"));
            ProgressText.Text = L10n.T("progress.cancelled");
        }
        else
        {
        ...
        // Закрываем секцию попытки — это инкрементит FailedAttempts (если нужно)
        // и пишет [Summary] в файл, ДО отправки телеметрии.
        attemptSw.Stop();
        App.SessionLog.EndAttempt(new AttemptOutcome
        {
            Ok = ok,
            Failed = fail,
            Cancelled = cancelled,
            Duration = attemptSw.Elapsed
        });

        // Send Telemetry (содержит все попытки в этой и предыдущих сессиях)
        _ = App.Telemetry.SendLogAsync();

        // Окно с итогами + связь со мной (только когда установка дошла до конца)
        if (!cancelled)
        {
            var done = new InstallCompleteDialog(ok, fail) { Owner = this };
            done.ShowDialog();
        }
    }
    ...
    {
        if (_installCts is { IsCancellationRequested: false } cts)
        {
            CancelInstallBtn.IsEnabled = false;
            ProgressText.Text = L10n.T("progress.cancelling");
        ...
        CancellationToken ct)
    {
        ...
        async Task<string> Acquire(string url, string suggestedName, string expectedHash, string label)
        {
            ...
            if (!string.IsNullOrEmpty(fileId))
            {
                ...
                if (cached != null)
                {
                    ...
                    else if (App.Cache.HasValidHashMarker(fileId, suggestedName, expectedHash))
                    {
                        // Уже проверяли этот файл с этим же хэшем — не пересчитываем 3 ГБ.
                        hashOk = true;
                    }
                    else if (VerifyHash(cached, expectedHash))
                    {
                        App.Cache.WriteHashMarker(fileId, suggestedName, expectedHash);
                        hashOk = true;
                    }
                    else hashOk = false;
            ...
            var dl = await downloader.DownloadAsync(url, pluginSrcDir, progress, ct,
            ...
            if (!string.IsNullOrEmpty(fileId))
            {
                try
                {
                    App.Cache.Put(fileId, desired, suggestedName);
                    if (!string.IsNullOrEmpty(expectedHash))
                        App.Cache.WriteHashMarker(fileId, suggestedName, expectedHash);
                }
                catch { }
        ...
        {
            // Скачиваем файлы плагина параллельно — сеть не блокирующий ресурс,
            // а Google Drive отдаёт каждому соединению независимый канал.
            // Порядок результата сохраняем через индекс — install_steps зависят
            // от порядка (особенно у Red Giant: 1_Maxon → 2_RedGiant → 3_Unlocker).
            var jobs = new List<Task<(int order, DownloadedFile df)?>>();
            int idx = 0;
            foreach (var f in plugin.Files)
            {
                ...
                var url = f.GoogleDriveUrl;
                var hash = f.Hash;
                var type = f.Type;
                var target = f.TargetPath;

                jobs.Add(Task.Run(async () =>
                {
                    var path = await Acquire(url, fname, hash, $"{type}: {fname}");
                    return ((int order, DownloadedFile df)?)
                        (order, new DownloadedFile(path, fname, type, target));
                }, ct));
            }

            var done = await Task.WhenAll(jobs);
            foreach (var item in done.Where(x => x.HasValue).OrderBy(x => x!.Value.order))
                result.Add(item!.Value.df);
        }
    ...
    {
        // Если на плагин есть встроенный рецепт (нативная установка без .bat),
        // отдаём приоритет ему — один архив, шаги выполняются по порядку
        // как один плагин в одной транзакции.
        if (!string.IsNullOrEmpty(plugin.RunAfterExtract) && files.Count == 1)
        {
            var only = files[0];
            var onlyType = only.Type == PluginType.Auto ? InferType(only.FileName) : only.Type;
            if (onlyType == PluginType.Archive && BuiltInRecipes.Has(plugin.Name))
            {
                var root = plugin.RunAfterExtract
                    .Replace('\\', '/')
                    .TrimStart('/')
                    .Split('/', 2)[0];
                var recipe = BuiltInRecipes.TryBuildStepsJson(
                    plugin.Name,
                    "{SRC_DIR}/" + only.FileName,
                    root);
            ...
            switch (actualType)
            {
                case PluginType.Archive:
                    ...
                    if (!string.IsNullOrEmpty(plugin.RunAfterExtract))
                    {
                        ...
                        if (plugin.NeedsAeVersion)
                            steps.Add(new { type = "run_exe", path = $"{stageDir}/{inner}", args = new[] { "{AE_VERSION}" }, wait = true });
                        else
    ...
    private static SessionLogLevel MapLevel(LogLevel l) => l switch
    {
    ...
    private string? _sessionJsonLogPath;

    /// <summary>
    /// Возвращает путь к JSONL-логу текущей сессии установки. Создаётся один раз
    /// за InstallAsync и переиспользуется для всех плагинов в этом сеансе.
    /// </summary>
    private string? GetSessionJsonLogPath()
    {
        if (_sessionJsonLogPath != null) return _sessionJsonLogPath;
        try
        {
            ...
            _sessionJsonLogPath = System.IO.Path.Combine(dir, $"install_{stamp}.jsonl");
        }
        catch { _sessionJsonLogPath = null; }
        return _sessionJsonLogPath;
    }
    ...
    {
        InstallAllBtn.Visibility = busy ? Visibility.Collapsed : Visibility.Visible;
        CancelInstallBtn.Visibility = busy ? Visibility.Visible : Visibility.Collapsed;
        CancelInstallBtn.IsEnabled = busy;
    ...
    private void AppendLog(string line) => AppendLog(line, mirrorToSession: true);

    private void AppendLog(string line, bool mirrorToSession)
    {
        HideLogEmptyHint();
        ...
        // Зеркалим в session-log как Info (no-op вне активной попытки).
        // ctx.UiLog отключает зеркалирование — там уже работает SessionSink с уровнями.
        if (mirrorToSession)
        {
            try { App.SessionLog.Info(line); } catch { /* best-effort */ }
        }
    }
    ...
    private void ConfigPathLink_Click(object sender, MouseButtonEventArgs e)
    {
        try
        {
            var root = Path.GetDirectoryName(_configService.ConfigsDirectory) ?? _configService.ConfigsDirectory;
