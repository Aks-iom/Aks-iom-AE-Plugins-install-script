...
public partial class App : Application
{
    ...
    public static SessionLog SessionLog => Services.GetRequiredService<SessionLog>();
    public static TelemetryManager Telemetry => Services.GetRequiredService<TelemetryManager>();
    public static PortableAutoUpdater Updater => Services.GetRequiredService<PortableAutoUpdater>();
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // Фоновая проверка обновлений (не блокирует UI; любые ошибки глушатся внутри).
    ...
    private static async Task CheckForUpdatesAsync(Window owner)
    {
        ...
        {
            var updater = Updater;
            // force: true — спрашиваем на каждый заход, даже если пользователь когда-то
            // нажал «Нет» для этой версии. IgnoreVersion больше не глушит автопроверку.
            var info = await updater.CheckAsync(force: true).ConfigureAwait(true);
            if (info is null) return;
            if (updater.IsInstallationRunning) return;

            owner.Dispatcher.Invoke(() =>
            {
                // Повторная проверка уже в UI-потоке: пользователь мог начать
                // установку плагинов между CheckAsync и показом диалога.
                if (updater.IsInstallationRunning) return;

                var dlg = new UpdateAvailableDialog(updater, info) { Owner = owner };
                dlg.ShowDialog();
                // По требованию: диалог появляется на каждый запуск. IgnoreVersion
                // больше не выставляем — отказ действует только в рамках текущей сессии.
            });
        }
        catch { /* апдейтер никогда не валит приложение */ }
    }
