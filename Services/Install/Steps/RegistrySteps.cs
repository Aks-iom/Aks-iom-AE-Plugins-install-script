...
public class EnableCepDebugStep : IInstallStep
{
    ...
    public StepResult Execute(InstallContext ctx)
    {
        ...
        // Целевая CSXS-ветка для текущей версии AE — её отсутствие критично,
        // даже если другие CSXS-ветки получили ключ (для CEP-плагина важна
        // именно та, под которую он собран).
        var expectedCsxs = CsxsForAeVersion(ctx.AeVersion);
        ...
        foreach (var csxs in CsxsBranches)
        {
            ...
            {
                skipped.Add($"{csxs}({ex.GetType().Name})");
            }
        }
        if (written.Count == 0)
            return StepResult.Fail($"enable_cep_debug: ни одна CSXS-ветка недоступна ({string.Join(", ", skipped)})", arts);

        if (expectedCsxs != null && !written.Contains(expectedCsxs))
            return StepResult.Fail(
                $"enable_cep_debug: ключ записан в {string.Join(", ", written)}, но НЕ в {expectedCsxs} (нужно для AE {ctx.AeVersion})", arts);

        ctx.Log($"   ✓ CEP debug включён в: {string.Join(", ", written)}");
        if (skipped.Count > 0)
            ctx.LogDebug($"enable_cep_debug: пропущены ветки {string.Join(", ", skipped)}");
        return StepResult.Ok(arts);
    }

    /// <summary>
    /// Маппинг версии AE → версия CSXS (CEP host).
    /// Источник: <see href="https://github.com/Adobe-CEP/CEP-Resources"/>.
    /// </summary>
    private static string? CsxsForAeVersion(string aeVersion) => aeVersion switch
    {
        "2022" => "CSXS.11",
        "2023" => "CSXS.11",
        "2024" => "CSXS.12",
