...
public class GoogleDriveDownloader : IDisposable
{
        ...
        Action<string>? log = null)
    {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            {
                lastError = ex;
                long got = partPath != null && File.Exists(partPath) ? new FileInfo(partPath).Length : 0;
                var delaySec = Math.Min(60, (int)Math.Pow(2, attempt));
                log?.Invoke($"   ⚠ Сетевой сбой (попытка {attempt}/{maxAttempts - 1}): {ex.Message.Trim()} — повтор через {delaySec}с, скачано {got / (1024 * 1024)} МБ");
