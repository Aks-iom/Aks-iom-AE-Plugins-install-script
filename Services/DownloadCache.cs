...
public class DownloadCache
{
    ...
    /// <summary>
    /// Проверяет, есть ли «доказательство» уже выполненной проверки хэша
    /// (маркер-файл рядом с кэшированным файлом). Если есть — повторный хэш
    /// можно не считать (для многогигабайтных архивов это критично).
    /// </summary>
    public bool HasValidHashMarker(string fileId, string desiredFileName, string expectedHash)
    {
        if (string.IsNullOrEmpty(fileId) || string.IsNullOrEmpty(expectedHash)) return false;
        var marker = MarkerPath(fileId, desiredFileName, expectedHash);
        return File.Exists(marker);
    }

    /// <summary>Создаёт маркер успешной проверки хэша рядом с кэшированным файлом.</summary>
    public void WriteHashMarker(string fileId, string desiredFileName, string expectedHash)
    {
        if (string.IsNullOrEmpty(fileId) || string.IsNullOrEmpty(expectedHash)) return;
        try
        {
            var marker = MarkerPath(fileId, desiredFileName, expectedHash);
            var dir = Path.GetDirectoryName(marker);
            if (!string.IsNullOrEmpty(dir)) Directory.CreateDirectory(dir);
            File.WriteAllText(marker, expectedHash.ToLowerInvariant());
            try { File.SetAttributes(marker, FileAttributes.Hidden); } catch { }
        }
        catch { /* best-effort */ }
    }

    private string MarkerPath(string fileId, string desiredFileName, string expectedHash)
    {
        var dir = Path.Combine(CacheDir, Sanitize(fileId));
        var stamp = expectedHash.ToLowerInvariant();
        if (stamp.Length > 16) stamp = stamp[..16];
        return Path.Combine(dir, $".{Sanitize(desiredFileName)}.{stamp}.ok");
    }
