    ...
    public List<PluginConfig> LoadAll()
    {
        ...
        foreach (var file in Directory.EnumerateFiles(_configsDir, "*.json"))
        {
            try
            {
                ...
                if (cfg != null)
                {
                    // Fail-safe: если в JSON флаг не выставлен, но имя совпадает с
                    // одним из канонических built-in — всё равно считаем встроенным.
                    if (!cfg.IsBuiltIn && PluginConfig.BuiltInNames.Contains(cfg.Name, StringComparer.OrdinalIgnoreCase))
                        cfg.IsBuiltIn = true;
                    list.Add(cfg);
                }
            }
