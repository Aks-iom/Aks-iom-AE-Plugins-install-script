...
public class Plugin
{
    ...
    /// <summary>
    /// True, если bat/exe-инсталлеру нужно передать год выбранной версии AE
    /// первым аргументом (например, Saber.bat — без аргумента молча ставит AE 2020).
    /// </summary>
    public bool NeedsAeVersion { get; set; }
...
public class PluginConfig
{
    ...
    /// <summary>
    /// Конфиг был импортирован из bundled-каталога (Catalogs/plugins.json,
    /// Catalogs/extended_list.json) — пользователь не должен мочь удалять
    /// из него плагины или удалить весь конфиг целиком. Редактирование
    /// отдельных полей разрешено.
    /// </summary>
    public bool IsBuiltIn { get; set; }

    /// <summary>Канонические имена встроенных каталогов — используются как fail-safe
    /// при загрузке: даже если в JSON флаг отсутствует, конфиги с этими именами
    /// всегда трактуются как built-in.</summary>
    public static readonly string[] BuiltInNames = { "Основной набор", "Расширенный список" };
}
